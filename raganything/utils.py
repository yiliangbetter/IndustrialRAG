...
async def snapshot_kg_recovery_anchors(
    ...
    LightRAG ``merge_nodes_and_edges`` replaces these recovery anchors with
    candidates from the *current* ``chunk_results`` only (1.4.15 Phase 3
    replace-upsert; later LightRAG Phase 0 write-ahead). Multimodal merge
