...
class ProcessorMixin:
    ...
    def _join_caption_and_body(self, caption: Any, body: str) -> str:
        cap = self._coerce_textish(caption)
        body = (body or "").strip()
        if cap and body:
            return f"{cap}\n{body}"
        return cap or body
    ...
    def _plaintext_from_mineru_blocks(self, items: List[Dict[str, Any]]) -> str:
        """Recover plaintext from MinerU v1/v2 blocks, including list/code/chart/tables.

        Used when multimodal processors are skipped (text-first KG / embedding-only)
        so MinerU ``list`` / ``code`` / ``chart`` / ``table`` blocks are not dropped.
        """
        parts: List[str] = []
        for item in items:
            ...
            if block_type == "text" and isinstance(item.get("text"), str):
                ...
                if isinstance(content, dict):
                    ...
                    cap = content.get("code_caption") or content.get(
                        "algorithm_caption"
                    )
                else:
                    s = self._coerce_textish(
                ...
                s = self._join_caption_and_body(cap, s)
            elif block_type == "chart":
                if isinstance(content, dict):
                    s = self._coerce_textish(content.get("content"))
                    cap = content.get("chart_caption") or content.get("image_caption")
                else:
                    s = self._coerce_textish(item.get("content"))
                    cap = item.get("chart_caption") or item.get("image_caption")
                s = self._join_caption_and_body(cap, s)
            elif block_type == "table":
                if isinstance(content, dict):
                    s = self._coerce_textish(content.get("html") or content.get("body"))
                ...
                s = self._join_caption_and_body(cap, s)
                footnote = self._coerce_textish(item.get("table_footnote"))
                if footnote:
                    s = f"{s}\n{footnote}" if s else footnote
            elif block_type == "equation":
                latex = self._coerce_textish(item.get("latex"))
                desc = self._coerce_textish(item.get("text"))
                if isinstance(content, dict):
                    if not latex:
                        latex = self._coerce_textish(content.get("latex"))
                    if not desc:
                        desc = self._coerce_textish(content.get("text"))
                if latex and desc:
                    s = f"{desc}\n{latex}"
                else:
                    s = latex or desc
            elif block_type == "image":
                s = self._coerce_textish(item.get("image_caption"))
                if isinstance(content, dict) and not s:
                    s = self._coerce_textish(content.get("image_caption"))
                footnote = self._coerce_textish(item.get("image_footnote"))
                if isinstance(content, dict) and not footnote:
                    footnote = self._coerce_textish(content.get("image_footnote"))
                if s and footnote:
                    s = f"{s}\n{footnote}"
                elif footnote:
                    s = footnote
        ...
        try:
            ...
            if self.config.allow_embedding_only_ingestion:
                if file_name is None:
                    file_name = self._get_file_reference(file_path)
                # Multimodal processors will not run — fold list/code/chart/table
                # (and other recoverable blocks) into the text stream.
    ...
    async def insert_content_list(
        ...
        # blocks (MinerU 3 list/code/chart/table, v2 nested prose, etc.) into the
        # text stream — even if type=text prose already made text_content non-empty.
