    ...
    async def process_document_complete_lightrag_api(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Additional parameters for parser (e.g., lang, device, start_page, end_page, formula, table, backend, source)
        """
        # Citation/display name may be basename, but the pre-status key must be unique
        # per absolute path. Default use_full_path=False would otherwise collide
        # doc-pre-<basename> across different directories and clobber HANDLING/FAILED.
        file_name = self._get_file_reference(file_path)
        try:
            abs_file_path = str(Path(file_path).resolve())
        except Exception:
            abs_file_path = str(file_path)
        doc_pre_id = f"doc-pre-{hashlib.md5(abs_file_path.encode()).hexdigest()}"
