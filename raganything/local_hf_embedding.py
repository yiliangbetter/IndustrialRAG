...
def make_local_hf_embedding_func(
    ...
    def _encode_sync(text_list: list[str]) -> np.ndarray:
        # LightRAG may call this concurrently. Serialize lazy construction and
        # encode() on the shared SentenceTransformer to avoid multi-load OOM and
        # thread-unsafe tokenizer/inference failures.
        with model_lock:
            model = _get_model()
            vecs = model.encode(
                text_list,
