...
调试检索上下文仍建议用 CLI：[`rag_pipeline_parse_graph_chat.py`](../scripts/rag_pipeline_parse_graph_chat.py) 的 `--query-only`。
...
| `rag_pipeline_parse_graph_chat.py --query-only` | 启动服务后直接对话；检索调试也用 CLI |
