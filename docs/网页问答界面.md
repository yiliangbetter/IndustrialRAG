...
页面已适配手机竖屏：宽度 ≤ 768px 时自动隐藏侧栏（灌库与安装向导仅电脑端提供），进入纯问答布局——顶部紧凑标题栏（含服务就绪状态圆点）、底部输入栏，并适配 iPhone 安全区。

手机与电脑需在同一局域网（如连同一 WiFi）。**推荐直接改根目录 `.env`**（启动时 `.env` 会覆盖 shell 环境变量）：
...
启动后终端会打印「手机访问」链接（形如 `http://192.168.x.x:8765/`），手机浏览器直接打开即可问答。若打不开，请检查 Windows 防火墙是否允许 Python 的入站连接。

> 说明：默认 `RAG_WEB_HOST=127.0.0.1` 仅本机可访问，手机无法连接；设为 `0.0.0.0` 后局域网内任何设备均可访问，请在可信网络环境下使用。

## 访问密钥保护（对外分享必配）

服务本身无登录验证，灌库/清空知识库等接口若暴露到公网会有风险。在 `.env` 设置 `RAG_WEB_ACCESS_KEY` 即可启用门控：

```ini
# .env
RAG_WEB_ACCESS_KEY=<随机密钥>
```

启用后：

- 未验证时页面返回 401 登录卡片，所有 `/api/*` 返回 401；
- 分享带密钥的链接：`http://<地址>:<端口>/?key=<密钥>`，首次打开验证成功后写入 Cookie（30 天），后续无需重复输入；
- `/static/*` 静态资源不受影响；不设置该变量则行为与旧版完全一致。

启动时终端会打印带密钥的分享链接，直接转发给他人即可。

## 外网分享（内网穿透）

电脑在内网时，可用隧道工具生成公网 HTTPS 链接，转发到本机 8765 端口；对方点链接即用，无需装任何软件。前提是**先启用访问密钥**。

| 工具 | 特点 |
|------|------|
| [cpolar](https://www.cpolar.com/) / natapp | 国内服务，速度快；需注册（有免费套餐），免费版域名可能随机变化 |
| cloudflared（Cloudflare Tunnel） | 免费稳定、无需账号即可临时使用（trycloudflare.com 域名），国内部分网络访问不稳定 |
| ngrok | 国外成熟服务，国内直连可能受限 |

以 cloudflared 免注册临时隧道为例：

```powershell
# 下载 cloudflared.exe 后（https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/downloads/）
cloudflared tunnel --url http://127.0.0.1:8765
```

终端会输出形如 `https://xxxx-xxxx.trycloudflare.com` 的地址，把它拼上密钥发给对方：

```text
https://xxxx-xxxx.trycloudflare.com/?key=<你的密钥>
```

注意事项：

- 隧道程序运行期间链接才有效；电脑关机或服务停止后需重新生成/重启隧道。
- 免费版临时域名每次启动会变；cpolar/ngrok 付费可固定域名。
- 长期稳定对外服务建议改为云服务器部署（需迁移知识库与模型，另议）。
...
| `RAG_WEB_ACCESS_KEY` | 空（不启用） | 访问密钥；设置后页面与 API 需带 `?key=<密钥>` 或 Cookie 验证 |
...
调试检索上下文仍建议用 CLI：[`dump_query_context.py`](../scripts/dump_query_context.py)（见 [`chunk-0d30-检索调试记录.md`](chunk-0d30-检索调试记录.md)）。
