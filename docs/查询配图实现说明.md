...
> **批测基线**（2026-07-06）：[`logs/web_path_q1_17/20260706_110213_all.md`](../logs/web_path_q1_17/20260706_110213_all.md) — 文字 **17/17**，配图 **13/17**（Q15 四机 **4/4 ✓**；失败 Q3、Q11、Q16、Q17）
...
| 路径 | 触发条件 | 行为 |
|------|----------|------|
| **统一多 target** | `RAG_IMAGE_CHUNK_LOCALITY=1` 且 `extract_figure_targets` ≥2 个 target，且 kind 为 `machine_component_pair` 或（`machine_bullet` + 多机 query/citation） | `_refs_from_unified_figure_targets` → 每 target 调 `_figure_for_target` |
| **Legacy + 可选 augment** | 其余单题 / 单手册列举 | `_refs_from_retrieved_docs_text` 等；单 target 可 `_refs_from_unified_single_target_augment` 合并一张 |
...
# kind: "single" | "machine_bullet" | "machine_component_pair"
```
...
2. anchor ← _best_anchor_chunk(manual_chunks, anchor_text, cite_pool=scoped_cite)
     · _resolve_doc_with_order_index 必填
     · 惩罚 [Table]/TOC/超长块；加分短节标题、citation overlap
3. figure ← anchor 含 [图片] → anchor
           elif _figure_doc_for_anchor_neighbor(anchor, window=RAG_IMAGE_CHUNK_LOCALITY_WINDOW)
           elif pair：_best_figure_doc_for_component + listing align
4. align gate ← text_term_alignment_symmetric(anchor_text, label/context)
           若 < RAG_IMAGE_MIN_REF_ALIGN → 丢弃
5. 返回 ref + meta（anchor_id, figure_source, align_score）
```
