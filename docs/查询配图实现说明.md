...
> **关联文档**：[`查询配图设计方案.md`](查询配图设计方案.md) · [`测试例参考答案.md`](测试例参考答案.md) · [`问答全流程图.md`](问答全流程图.md)  
...
> **状态**：已与 Web 前台路径对齐；Q1–Q13 批测（`scripts/run_web_path_q1_13.py`）验证通过
...
**原则**：不在 rerank 全池、不在 MinerU 原始 JSON 上临时补图；**只** 从 cited chunk 的正文里 `extract_image_refs_from_context`。
...
| `no_inline_placements` | 已选图但算不出插入位置（旧逻辑；现已加 fallback，少见） |
...
| `scripts/run_web_path_q1_13.py` | 模拟 Web 全路径 + 对照 `测试例参考答案.md` 批测 |
...
| 批测 | `scripts/run_web_path_q1_13.py` | Web 路径回归 |
...
| 展示 | 答案下方横排 | **段落后 inline `<figure>`** |
| 张数 | 硬限 4 | 列举题按 target，cap 24 |
| 扫图范围 | 曾用 rerank 全池 / content_list 补图 | **仅 cited chunk inline 块** |
| placement | 无 | **`build_inline_placements`**（含语义锚点 + 末尾兜底） |
