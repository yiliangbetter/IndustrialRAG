...
**阶段 4 全量结果**（`20260614_005940_all.md`）：**16/17**（文字 17/17）。**Q16 已收尾**：Web 路径手测稳定（三本手册各 1 张三联件图）；全量批测偶发 FAIL 为 grader 对非 `-` 列表答案解析 + citation 方差，不阻塞主路径。Q5 gate 对齐后稳定；Q17 pair 列举稳定 PASS。
