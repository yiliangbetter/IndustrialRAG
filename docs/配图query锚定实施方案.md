# 配图 Query 锚定实施方案（阶段 4 已完成）

> **目的**：记录 2026-06-12 讨论的配图改进方向与分步计划，便于追溯与回退。  
> **关联**：[`查询配图设计方案.md`](查询配图设计方案.md) · [`查询配图实现说明.md`](查询配图实现说明.md) · [`测试例参考答案.md`](测试例参考答案.md)  
...
| 项 | 内容 |
|----|------|
| **Git commit** | `94ffcd9` — `feat(images): phase 2 pool-first span targets for listing figures` |
| **批测结果** | 全量 Q1–Q17：**16/17**；报告 `logs/web_path_q1_17/20260613_020629_all.md` |
| **相对阶段 1** | Q17 配图达标（≥3 张）；Q13 文字方差（缺「高温润滑脂」）**暂不处理**，非配图回归 |
| **主要改动** | `_span_keep_listing_targets`、`_listing_topics_from_pool`、`_order_listing_targets_by_hints`；列举题 span 来自池内 `保养内容：` + citation kept，答案加粗仅排序 |
| **子集回归** | `20260613_014846` **5/5**（Q5/Q9/Q11/Q15/Q17） |

回退阶段 2：`git revert 94ffcd9` 或 `git checkout 4837143` / `ab4ed96` 后 cherry-pick 所需提交。
...
### 阶段 2：收敛 answer-span 扩池逻辑（中风险）✅ 已完成
...
| 2.2 | 列举题 span_keep：span 优先来自 **query 保养条目**（检索正文 `保养内容：`）与 **citation kept** 的交集；答案加粗仅作 **次序提示** | Q11 稳定 3 张残胶图 | ✅ `_span_keep_listing_targets` / `_listing_topics_from_pool` |
| 2.3 | 多机型周期题（Q15）：维持 **机型 span + query term** 过滤（`discriminative_terms`），不改为纯 query 漫游 | Q15 稳定 4 张电控板图 | ✅ |

**批测**：子集 `20260613_014846` **5/5**；全量 `20260613_020629` **16/17**（Q13 文字方差缺「高温润滑脂」；Q17 配图 3 张达标）。

### 阶段 3：Q17 跨手册列举补全（较高复杂度）✅ 已完成

**目标**：答案列举的各部件，在对应手册段落有图则应配上；仍不脱离 citation/query 边界。

| 步骤 | 改动要点 | 验收 | 状态 |
|------|----------|------|------|
| 3.1 | 列举题：每个 **query 对齐的保养条目**（非答案加粗）在 **citation pool ∪ 同手册同节邻段** 内取 1 张图 | 智能机：压带轮 / 仿形 / 涂胶轴 / 电机（若有） | ✅ `_expand_pool_with_same_section_neighbors`、`_supplement_component_listing_figure_chunks` |
| 3.2 | 跨手册：citation 已引用手册来源（References PDF）限制补图 **source_hints** | 高速自动机：激光出光口图仅来自该手册 | ✅ `_cited_manual_hints_from_answer`、`_source_hints_for_images` |
| 3.3 | span 在 pool 内未命中时，**受限** content_list bbox 补图：仅 `source_hints` + label 与 **query 条目** 对齐，且答案正文含对应机型/部件 **弱一致** | Q17 ≥3 张且不乱配 | ✅ `_supplement_cited_listing_refs_from_content_lists` + `context_text_for_image` 无 caption 图 |

**批测**：单题 `20260613_111708_q17.md` **PASS** 4 图；子集见变更记录。批测报告时间戳改为 `datetime.now().astimezone()`（本地时区）。

### 阶段 3 延伸：跨机型 pair 选图 / 落位（Q16/Q17）

| 步骤 | 改动要点 | 验收 | 状态 |
|------|----------|------|------|
| B | `_supplement_pair_driven_refs_from_docs`：按答案 `(机型, 部件)` 从扩池补图；`source_key` 参与机型 hint | Q17 各手册各部件有图 | ✅ |
| C | `_load_figure_chunks_for_manual_paths`（KV 补池）、`supplement_multi_manual_listing_chunks`（答前 rerank） | citation 漏手册时仍可补图 | ✅ |
| pair placement | `_pair_component_ref_align`（`保养内容` + 节标题）、机型段落 scoped 锚点；去掉 `cross_manual_order` 乱序兜底 | Q17 图嵌在对应 bullet 下 | ✅ |
| 前端 | `findBlockForAnchor` 去 markdown 符号；内联失败时图廊兜底 | Web 不再「有 placements 但 0 图」 | ✅ |

### 阶段 4：批测、文档与开关 ✅ 已完成

| 步骤 | 改动要点 | 验收 | 状态 |
|------|----------|------|------|
| 4.1 | 全量 `run_web_path_q1_17.py`；记录报告路径 | `logs/web_path_q1_17/20260613_152851_all.md` **16/17**（文字 17/17） | ✅（Q16 配图见下） |
| 4.2 | 更新 `查询配图设计方案.md` §0、`查询配图实现说明.md` 与本文一致 | 设计 / 实现文档同步 | ✅ |
| 4.3 | `config/env.example` query 节锚开关 | `RAG_IMAGE_QUERY_SECTION_ANCHOR` | ✅ 阶段 1 |

**阶段 4 批测摘要**（`20260613_152851_all.md`）

| 结果 | 说明 |
|------|------|
| **16/17** | Q1–Q15、Q17 文字+配图均 PASS |
| **Q16 ✗** | 文字 PASS；配图 2/3，`pair_missing: 高速自动封边机/三联件油雾器`（见下） |
| **Q17 ✓** | 涂胶轴/涂胶电机分条 + pair 落位稳定 |

**Q16 未过原因（记录为例外，非阶段 1–3 回归）**

1. 本次 citation 后仅 **1 个 chunk** 进入 `answer_topics` 扫图，未从 KV/补池拉全三本封边机手册的三联件图（缺 **自动封边机** 手册图；仅双端 + 高速自动 2 张）。
2. 批测 `_image_matches_machine` 仍用 `token in blob` 子串，**「自动封边机」可误匹配「高速自动封边机」路径**，导致 pair 计分把高速自动图算给自动封边机，报 `高速自动` 缺失（与 Web 展示无关，属批测脚本待收紧）。
...
| 项 | 内容 |
|----|------|
| **Git commit** | 工作区含阶段 3 延伸 + 阶段 4 文档（基于 `f4da94c` 之后未提交改动） |
| **批测结果** | 全量 Q1–Q17：**16/17**（文字 **17/17**）；报告 `logs/web_path_q1_17/20260613_152851_all.md` |
| **配图失败** | Q16（三联件油雾器跨机型 1:1，见上） |
| **配图达标** | Q5/Q9/Q11/Q15/Q17 等；Q17 pair 内联 4+ 张 |
| **主要改动** | pair 选图/落位、`source_key` 机型匹配、残胶题答案 prompt 分条、Web 锚点与图廊兜底 |
...
| 2026-06-13 | `94ffcd9` | 阶段 2：`_span_keep_listing_targets` 池内保养条目优先、答案加粗仅排序；Q17 配图改善 |
| 2026-06-13 | — | 阶段 2 批测：子集 `20260613_014846` **5/5**；全量 `20260613_020629` **16/17**（Q13 文字方差暂缓） |
| 2026-06-13 | `87fdf69` | 阶段 3：Q17 跨手册列举 + content_list 受限补图；批测报告时间戳改本地时区 |
| 2026-06-13 | — | 阶段 3 批测：单题 `20260613_113927_q17.md` **PASS** 3 图；子集 Q5/9/11/15 稳定，Q17 受 LLM 答案方差影响 |
| 2026-06-13 | — | 阶段 3 延伸：pair 选图/落位、KV 补池、Q17 残胶 prompt 分条、Web 锚点修复 |
| 2026-06-13 | — | 阶段 4 批测：`20260613_152851_all.md` **16/17**（Q16 配图例外）；文档同步 |
