...
- **重排序（rerank）**：LightRAG 在 **`RERANK_BY_DEFAULT=true`** 时会对检索到的 chunk 做重排，但必须在构建 **`LightRAG`** 时提供 **`rerank_model_func`**。本仓库的 **`scripts/rag_pipeline_parse_graph_chat.py`**（以及依赖其 **`_build_rag`** 的 **`dump_query_context.py`**）已按环境变量注入该函数。请在 `.env` 中配置（详见根目录 **`env.example`** 中 `### Rerank` 段）：
...
**推荐脚本**：`scripts/dump_query_context.py`（完整工程 / 后续 tooling PR 提供；只跑 `aquery_data`、**不调 LLM**，导出实体/关系/chunk，用于判断「表内容有没有进检索」）。

```powershell
# 未写问题时可用环境变量 DUMP_QUERY_DEFAULT；输出默认 docs/query_context_dump.txt
...
若当前检出**没有**该文件，可用主脚本 **`--query-only`** 做端到端自检兜底：
