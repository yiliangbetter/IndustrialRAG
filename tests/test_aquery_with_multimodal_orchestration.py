...
Open PR #77 covers multimodal description builders and the empty-content
text fallback. Cache key normalization is covered by PR #94. This module
covers the remaining high-risk orchestration: init fail-closed, cache hit
short-circuit, cache write + persist after a successful enhanced query,
and cache write soft-failure isolation.
"""
...
async def test_aquery_with_multimodal_continues_when_cache_ops_fail():
    ...
    # Persist callback also fails — should still return the answer.
    ...
    assert any("Error accessing multimodal query cache" in d for d in query.logger.debugs)
