...
Strategy 3 recovers those payloads before the regex last-resort path, which can
silently drop structure even when a few fields match.
"""
...
class TestProgressiveQuoteFix:
    ...
    def test_robust_parse_uses_strategy_three_when_direct_and_cleanup_fail(self):
        ...
        # Regex fallback would still recover names; strategy 3 must keep
        # the nested entity_info object rather than flattening.
        assert "entity_info" in result
        assert set(result["entity_info"]) == {
            "entity_name",
            "entity_type",
            "summary",
        }
