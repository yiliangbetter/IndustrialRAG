...
Covers working_dir / parser_output_dir allowlisting, encode soft-failures
that must not invent VLM markers, missing files, and mixed safe/unsafe
paths in one prompt.
"""
...
@pytest.mark.asyncio
async def test_process_image_paths_allows_parser_output_dir_images(
...
async def test_process_image_paths_keeps_original_when_encode_returns_empty(
    ...
    monkeypatch.setattr("raganything.query.encode_image_to_base64", lambda _path: "")
