...
@pytest.mark.asyncio
async def test_union_kg_recovery_anchors_noop_without_doc_id_or_prior():
    full_entities = FakeKV({"doc-1": {"entity_names": ["Pump"], "count": 1}})
    full_relations = FakeKV({"doc-1": {"relation_pairs": [["Pump", "Valve"]], "count": 1}})

    await union_kg_recovery_anchors(
        full_entities, full_relations, None, {"entity_names": ["Alice"]}, None
    )
    await union_kg_recovery_anchors(full_entities, full_relations, "doc-1", None, None)

    assert (await full_entities.get_by_id("doc-1"))["entity_names"] == ["Pump"]
    assert (await full_relations.get_by_id("doc-1"))["relation_pairs"] == [["Pump", "Valve"]]
    assert full_entities.flushed == 0
    assert full_relations.flushed == 0
...
async def test_batch_merge_restores_anchors_wiped_by_phase0():
    """Hot-path multimodal merge: Phase 0/3 overwrite must not drop prior anchors."""
    ...
    async def fake_merge(**kwargs):
        # Reproduce LightRAG Phase 0/3 replacement semantics.
