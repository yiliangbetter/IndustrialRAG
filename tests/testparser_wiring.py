...
@pytest.mark.asyncio
async def test_parse_document_office_skips_none_method(monkeypatch, tmp_path):
    import raganything.processor as processor_module
...
@pytest.mark.asyncio
async def test_parse_document_routes_html_to_parse_html(monkeypatch, tmp_path):
    import raganything.processor as processor_module
    ...
    class FakeParser:
        def parse_html(self, **kwargs):
            called["parse_html"] = kwargs
            return [{"type": "text", "text": "html parsed", "page_idx": 0}]

        def parse_office_doc(self, **kwargs):
            called["parse_office_doc"] = kwargs
            return [{"type": "text", "text": "office parsed", "page_idx": 0}]
    ...
    assert "parse_html" in called
    assert "parse_office_doc" not in called
