...
def test_concurrent_language_switches_never_expose_empty_or_partial_snapshots():
    ...
    def reader():
        for _ in range(400):
            ...
            # While writers race, the sample key must be exactly one full
            # language snapshot — never a torn/partial write.
            if value not in VALID_MARKERS and "expert" not in str(value).lower():
                # English default is also acceptable if a reset raced in;
                # anything else indicates corruption.
                if value not in VALID_MARKERS:
                    # Allow English default text from the original registry.
...
def test_reset_prompts_under_concurrent_set_restores_consistent_english():
    ...
    reset_prompts()
    assert get_prompt_language() == "en"
    assert "expert" in PROMPTS[SAMPLE_KEY].lower() or "image" in PROMPTS[SAMPLE_KEY].lower()
