...
class TestGetOcr:
    ...
    def test_falls_back_to_empty_kwargs_then_raises_when_all_fail(self, monkeypatch):
        parser = PaddleOCRParser(default_lang="en")
        calls = []
...
class TestOcrRenderedPage:
    def test_saves_pil_to_temp_and_deletes_even_on_ocr_failure(self, monkeypatch):
        parser = PaddleOCRParser()
        saved_paths = []
