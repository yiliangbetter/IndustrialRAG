...
Open routing tests mock these methods; this file covers validation, unique
output dirs (#51), kwargs forwarding, and return wiring.
"""
...
class TestParseHtml:
    ...
    def test_xhtml_accepted_uses_unique_output_dir_and_returns_blocks(
        ...
        # #51: shared output_dir must be namespaced by path hash
        assert seen["output_dir"].parent == out
        assert seen["output_dir"].name.startswith("page_")
        assert len(seen["output_dir"].name.split("_")[-1]) == 8
        assert seen["read_args"] == (seen["output_dir"], "page")

    def test_default_output_dir_without_unique_subdir(self, monkeypatch, tmp_path):
        parser = DoclingParser()
...
class TestParseOfficeDoc:
    ...
    def test_docx_uses_unique_output_dir_and_forwards_kwargs(
        ...
        assert seen["output_dir"].parent == out
        assert seen["output_dir"].name.startswith("report_")
